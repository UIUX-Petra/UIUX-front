...
<nav class="navbar-container fixed top-0 left-0 bg-[var(--bg-primary)] bg-opacity-95 w-full h-auto shadow-md text-white z-50">
    <div class="max-w-screen-xl relative flex flex-wrap items-center justify-between mx-auto p-3">
        ...
        <div class="hidden md:flex items-center gap-4 ml-auto">
            ...
            <button id="theme-toggle" onclick="toggleTheme()" class="theme-toggle p-2 rounded-full" aria-label="Toggle theme">
                ...
                <div class="avatar-container relative">
                    ...
                    <div class="user-dropdown hidden absolute right-0 mt-2 w-60 bg-[var(--bg-secondary)] rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 py-1"
                        ...
                        <div class="px-4 py-3 border-b border-[var(--border-color)]">
                            ...
                            <span class="block text-sm font-medium text-[var(--accent-tertiary)]">{{ $username ?? 'User' }}</span>
                        </div>
                        <div class="py-1" role="none">
                            <a href="{{ route('seeProfile') }}" class="user-menu-item text-[var(--text-primary)] hover:text-[var(--accent-tertiary)] hover:bg-[var(--bg-card-hover)] block px-4 py-2 text-sm"
                                role="menuitem">
                                <i class="fa-solid fa-user mr-2"></i> Profile
                            </a>
                            <a href="{{ route('editProfile') }}" class="user-menu-item text-[var(--text-primary)] hover:text-[var(--accent-tertiary)] hover:bg-[var(--bg-card-hover)] block px-4 py-2 text-sm"
                                role="menuitem">
                                <i class="fa-solid fa-gear mr-2"></i> Settings
                            </a>
                            <a href="{{ route('logout') }}" class="user-menu-item text-[var(--text-primary)] hover:text-red-400 hover:bg-[var(--bg-card-hover)] block px-4 py-2 text-sm"
                                role="menuitem">
        ...
        @if (!session()->has('email'))
            ...
            <a href="{{ route('loginOrRegist') }}" class="md:hidden text-white bg-[var(--accent-tertiary)] hover:bg-[var(--accent-primary)] font-medium rounded-lg text-xs px-3 py-1.5 transition-all duration-300">
                Sign Up
            </a>
        @else
            <!-- Mobile User Menu -->
    ...
    <div id="mobile-menu" class="mobile-menu hidden flex-col gap-1 p-4 md:hidden bg-[var(--bg-secondary)] text-[var(--text-primary)] w-full border-t border-[var(--border-color)]">
        ...
        <a href="{{ route('home') }}" class="nav-link flex items-center px-3 py-2 rounded-md {{ request()->routeIs('home') ? 'active-nav' : '' }}">
            <i class="fa-solid fa-house mr-3"></i> Home
        </a>
        <a href="{{ route('popular') }}" class="nav-link flex items-center px-3 py-2 rounded-md {{ request()->routeIs('popular') ? 'active-nav' : '' }}">
            <i class="fa-solid fa-fire mr-3"></i> Popular
            <span class="nav-badge">Hot</span>
        </a>
        <a href="{{ route('viewAllTags') }}" class="nav-link flex items-center px-3 py-2 rounded-md {{ request()->routeIs('viewAllTags') ? 'active-nav' : '' }}">
            <i class="fa-solid fa-tags mr-3"></i> Tags
        </a>
        <a href="{{ route('viewAllUsers') }}" class="nav-link flex items-center px-3 py-2 rounded-md {{ request()->routeIs('viewAllUsers') ? 'active-nav' : '' }}">
            <i class="fa-solid fa-users mr-3"></i> Informates
        </a>
        <a href="{{ route('askPage') }}" class="nav-link flex items-center px-3 py-2 rounded-md {{ request()->routeIs('askPage') ? 'active-nav' : '' }}">
            <i class="fa-solid fa-question-circle mr-3"></i> Ask a Question
        </a>
        <a href="{{ route('user.leaderboard') }}" class="nav-link flex items-center px-3 py-2 rounded-md {{ request()->routeIs('user.leaderboard') ? 'active-nav' : '' }}">
        ...
        <div class="flex items-center justify-between p-2">
            <button id="mobile-theme-toggle" onclick="toggleTheme()" class="theme-toggle p-2" aria-label="Toggle theme">
                <i id="mobile-theme-toggle-icon" class="fa-solid fa-sun"></i>
            </button>
            
            @if (session()->has('email'))
                <div class="flex space-x-3">
                    <a href="{{ route('seeProfile') }}" class="text-[var(--text-primary)] hover:text-[var(--accent-tertiary)]">
                        <i class="fa-solid fa-user"></i>
                    </a>
                    <a href="{{ route('editProfile') }}" class="text-[var(--text-primary)] hover:text-[var(--accent-tertiary)]">
    ...
    <div class="flex flex-col h-full mt-16">
        ...
        <div class="mb-8 nav-section pb-6">
            ...
            <nav class="flex flex-col space-y-1">
                <a href="{{ route('home') }}" class="nav-link {{ request()->routeIs('home') ? 'active-nav' : '' }} text-[var(--text-primary)] py-2.5 text-sm pl-3 rounded-md flex items-center font-medium">
                    <i class="fa-solid fa-house mr-3 w-5 text-center"></i> Home
                </a>
                <a href="{{ route('popular') }}" class="nav-link {{ request()->routeIs('popular') ? 'active-nav' : '' }} text-[var(--text-primary)] py-2.5 text-sm pl-3 rounded-md flex items-center font-medium">
        ...
        <div class="mb-8 nav-section pb-6">
            ...
            <nav class="flex flex-col space-y-1">
                <a href="{{ route('viewAllUsers') }}" class="nav-link {{ request()->routeIs('viewAllUsers') ? 'active-nav' : '' }} text-[var(--text-primary)] py-2.5 text-sm pl-3 rounded-md flex items-center font-medium">
                    <i class="fa-solid fa-users mr-3 w-5 text-center"></i> Informates
                </a>
                <a href="{{ route('user.leaderboard') }}" class="nav-link {{ request()->routeIs('user.leaderboard') ? 'active-nav' : '' }} text-[var(--text-primary)] py-2.5 text-sm pl-3 rounded-md flex items-center font-medium">
