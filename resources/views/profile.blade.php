    ...
    <div class="text-gray-900 min-h-screen p-6 sm:p-10 mx-auto">
        ...
        <div class="w-full bg-white rounded-lg p-6 shadow-lg">
            ...
            <div class="flex flex-col sm:flex-row items-center sm:space-x-4">
                <img src="{{ $image ? asset('storage/' . $image) : 'https://via.placeholder.com/100' }}" alt="Profile Picture"
                    class="w-20 h-20 rounded-full mb-4 sm:mb-0">
                <div>
            ...
            <!-- Content Sections -->
            <div class="grid grid-cols-2 lg:grid-cols-12 gap-6 mt-8">
                <!-- Stats Section -->
                        ...
                        <div class="border border-[#7494ec] shadow-lg p-4 rounded-lg bg-white">
                         ...
                         <div class="border border-[#7494ec] shadow-lg p-4 rounded-lg bg-white">
                            <h2 class="text-[#7494ec] text-lg font-bold hover:underline">
                                <a href="{{ route('user.questions.list', ['id' => $currUser['id']]) }}">Questions</a>
                            </h2>
                            <div class="grid grid-cols-2 gap-4 mt-4">
                                <div class="text-[#7494ec]">
                                    {{ $currUser['questions_count'] ?? (isset($currUser['question']) && is_array($currUser['question']) ? count($currUser['question']) : 0) }} Questions
                                </div>
                            </div>
                        </div>
                    </div>
...
<div class="text-[var(--text-primary)] min-h-screen p-4 sm:p-6 lg:p-8 max-w-[70rem] justify-start items-start profile-container">
    <!-- Main Content -->
    <div class="w-full rounded-xl shadow-lg overflow-hidden profile-card">
        ...
        <div class="pt-20 sm:pt-6 sm:pl-48 px-4 sm:px-8 pb-6">
            <div class="flex flex-col sm:flex-row justify-between items-center sm:items-start">
                ...
                <div class="text-center sm:text-left mb-4 sm:mb-0">
                    <h2 class="text-[var(--text-primary)] text-3xl font-bold cal-sans-regular">{{ $currUser['username'] }}</h2>
                </div>
                
                <!-- Actions -->
                <div class="flex space-x-4">
                    <a href="{{route('editProfile')}}" class="px-4 py-2 btn-primary text-white rounded-lg flex items-center">
        ...
        <!-- Stats Bar -->
        <div class="bg-[var(--bg-card-hover)] px-6 py-4 flex justify-around border-t border-b border-[var(--border-color)]">
            <div class="text-center">
            ...
            <div class="text-center">
                <h3 class="text-[var(--text-primary)] text-xl font-bold">0</h3>
                <p class="text-[var(--text-muted)] text-sm">Following</p>
            </div>
            <div class="text-center">
                <h3 class="text-[var(--text-primary)] text-xl font-bold">0</h3>
                <p class="text-[var(--text-muted)] text-sm">Questions</p>
            </div>
            <div class="text-center">
                <h3 class="text-[var(--text-primary)] text-xl font-bold">0</h3>
                <p class="text-[var(--text-muted)] text-sm">Answers</p>
            </div>
        </div>
    </div>
        ...
        <div class="col-span-1 lg:col-span-3 space-y-6">
            ...
            <div class="profile-card p-6">
                ...
                <div class="flex flex-wrap gap-2">
                    <span class="profile-tag px-3 py-1 text-white bg-[var(--bg-shadow)] text-sm rounded cursor-pointer">angular</span>
                    <span class="profile-tag px-3 py-1 text-white bg-[var(--bg-shadow)] text-sm rounded cursor-pointer">html</span>
                    <span class="profile-tag px-3 py-1 text-white bg-[var(--bg-shadow)] text-sm rounded cursor-pointer">css</span>
                    <span class="profile-tag px-3 py-1 text-white bg-[var(--bg-shadow)] text-sm rounded cursor-pointer">javascript</span>
                </div>
            </div>
            ...
            <div class="profile-card p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="font-bold text-[var(--text-primary)]">0</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-[var(--text-secondary)]">Reached</span>
                        <span class="font-bold text-[var(--text-primary)]">0</span>
                    </div>
                    <div class="flex justify-between items-center">
                ...
                <div class="space-y-4">
                    <div class="profile-post p-4 rounded-lg">
                        <div class="flex items-start">
                            <div class="flex-1">
                                <a href="#" class="text-[var(--text-primary)] font-medium hover:underline text-lg">
                                    How does JVM Clojure store captured environments of closures under the hood?
                                </a>
                                <div class="flex flex-wrap gap-2 mt-2 mb-3">
                                    <span class="profile-tag px-2 py-0.5 text-white bg-[var(--bg-shadow)] text-xs rounded">clojure</span>
                                    <span class="profile-tag px-2 py-0.5 text-white bg-[var(--bg-shadow)] text-xs rounded">jvm</span>
                                    <span class="profile-tag px-2 py-0.5 text-white bg-[var(--bg-shadow)] text-xs rounded">closures</span>
                                </div>
                                <div class="flex items-center mt-3 space-x-6">
                                    <div class="flex items-center">
                                        <button class="px-3 py-1 btn-primary text-white text-sm rounded-lg mr-2 flex items-center">
                    ...
                    <div class="text-center py-8 bg-[var(--bg-card-hover)] rounded-lg">
                        ...
                        <p class="text-[var(--text-muted)] text-sm mt-1 mb-4">Write a new question to share your knowledge</p>
                        <a href="{{ route('askPage') }}" class="ask-question-btn {{ request()->routeIs('askPage') ? 'active-ask' : '' }} bg-gradient-to-r from-[#38A3A5] to-[#80ED99] text-black font-medium text-[0.75rem] p-2 rounded-lg items-center justify-center hover:shadow-lg hover:from-[#80ED99] hover:to-[#38A3A5] transform hover:scale-105 transition-all duration-200">
